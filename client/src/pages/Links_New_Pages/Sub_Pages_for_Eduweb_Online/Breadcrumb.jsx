...
export const Breadcrumb = ({ fieldname }) => {
    ...
    const filteredData = datas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogCategory === "About Eduweb Online"
        ).length !== 0
    );
  ...
  if (datas.length === 0) {
    return (
      <FormatForNoData
        department="Eduweb Online"
        department_link="/eduwebonline"
        fieldname={fieldname}
    ...
    <div className="w-full h-full flex justify-center items-center flex-col bg-gray-200 lg:mt-12 mt-[2.4rem] md:z-10">
      ...
      >
        <ol className="inline-flex items-center space-x-1 md:space-x-3">
          ...
          <li className="inline-flex items-center">
            <Link to={"/eduwebonline"} className="inline-flex items-center">
              <svg
                    ...
                    <ul className="w-full h-full flex justify-center items-center flex-col">
                      {filteredDatas.map((item) =>
                        item.writtenBlogsByAuthor.map((topic) => {
                          ...
                          let result = category.match("About Eduweb Online");
                          if (result) {
                            return (
                              <div
