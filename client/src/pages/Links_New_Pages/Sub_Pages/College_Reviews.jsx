...
export const College_Reviews = ({ aboutCollege_Reviews, filteredDatas, fieldname }) => {
  const [collegeReviewsDatas, setCollegeReviewsDatas] = useState([]);

  useEffect(() => {
    const filteredData = filteredDatas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogCategory.substring(6) === fieldname && field.blogSubCategory === aboutCollege_Reviews
        ).length !== 0
    );

    setCollegeReviewsDatas(filteredData);
  }, [filteredDatas]);
      ...
      <div className="w-full h-full flex justify-evenly items-start lg:flex-row flex-col bg-gray-300">
        <div className="w-full flex justify-center items-start">
          <div className="w-full flex justify-center items-center flex-col md:p-1 p-[0.3rem]">
            {collegeReviewsDatas.length > 0 ? (
              <ul className="w-full h-full flex justify-center items-center flex-col">
                {collegeReviewsDatas.map((item) =>
                  item.writtenBlogsByAuthor.map((topic) => {
                    if (topic.blogCategory.substring(6) === fieldname && topic.blogSubCategory === aboutCollege_Reviews) {
