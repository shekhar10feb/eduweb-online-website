...
export const Exams = ({ aboutExams, filteredDatas, fieldname }) => {
  const [examsDatas, setExamsDatas] = useState([]);

  useEffect(() => {
    const filteredData = filteredDatas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogCategory.substring(6) === fieldname && field.blogSubCategory === aboutExams
        ).length !== 0
    );

    setExamsDatas(filteredData);
  }, [filteredDatas]);
                ...
                {examsDatas.map((item) =>
                  item.writtenBlogsByAuthor.map((topic) => {
                    if (
                      ...
                      topic.blogSubCategory === aboutExams
                    ) {
