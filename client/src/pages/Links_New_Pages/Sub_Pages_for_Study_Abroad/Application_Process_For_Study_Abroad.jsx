...
export const Application_Process_For_Study_Abroad = ({
  aboutApplicationProcess,
  filteredDatas,
}) => {
  const [applicationProcessDatas, setApplicationProcess] = useState([]);

  useEffect(() => {
    const filteredData = filteredDatas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogSubCategory === aboutApplicationProcess
        ).length !== 0
    );

    setApplicationProcess(filteredData);
  }, [filteredDatas]);
      ...
      <div className="w-full h-full flex justify-evenly items-start lg:flex-row flex-col bg-gray-300">
        <div className="w-full flex justify-center items-start">
          <div className="w-full flex justify-center items-center flex-col md:p-1 p-[0.3rem]">
            {applicationProcessDatas.length > 0 ? (
              <ul className="w-full h-full flex justify-center items-center flex-col">
                {applicationProcessDatas.map((item) =>
                  item.writtenBlogsByAuthor.map((topic) => {
                    if (topic.blogSubCategory === aboutApplicationProcess) {
