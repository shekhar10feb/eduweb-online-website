...
export const Colleges_For_Study_Abroad = ({ aboutColleges, filteredDatas }) => {
  const [collegesDatas, setCollegesDatas] = useState([]);

  useEffect(() => {
    const filteredData = filteredDatas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogSubCategory === aboutColleges
        ).length !== 0
    );

    setCollegesDatas(filteredData);
  }, [filteredDatas]);
