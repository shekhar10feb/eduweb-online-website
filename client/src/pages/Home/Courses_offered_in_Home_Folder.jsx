...
import { CollegeFormat_for_List_of_Courses_Offered_in_Home_Folder } from "./CollegeFormat_for_List_of_Courses_Offered_in_Home_Folder.jsx";
import { Pre_Loader } from "./Pre_Loader";
import axios from "axios";

export const Courses_offered_in_Home_Folder = () => {
  ...
  useEffect(() => {
    const filteredData = datas.filter((item) =>
      item.courses_offered
        .split(", ")
        .filter((field) => field.toUpperCase() === uppercase_state)
    );
          ...
          <li>
            <div className="flex items-center">
              ...
              <h1 className="ml-1 text-sm font-medium md:ml-2 text-gray-500">
                {uppercase_state} Course Offered by Colleges
              </h1>
            </div>
          </li>
        </ol>
          ...
          <div className="w-full h-full flex justify-start items-start flex-col">
            <div className="w-full md:flex hidden justify-start items-center bg-white text-xl font-bold p-2">
              <h1 className="w-full lg:text-lg md:text-base underline text-wrap">
                List of Colleges offer {uppercase_state} Course
              </h1>
            </div>
                  ...
                  {filteredDatas.length > 0 ? (
                    <ul className="w-full h-full flex justify-center items-center flex-col">
                      {filteredDatas.map((data) => {
                        const link = `/All_Colleges/Course_Offered_By_Colleges/${data.name}`;
                        const text_from_All_Colleges = "All Colleges";
                        const goto_url = "/All_Colleges";
                        return (
                          <div
                            className="w-full h-full flex justify-center items-start flex-col bg-white sm:p-2 p-1 my-1 shadow-lg"
                            key={data._id}
                          >
                            <CollegeFormat_for_List_of_Courses_Offered_in_Home_Folder
                              data={data}
