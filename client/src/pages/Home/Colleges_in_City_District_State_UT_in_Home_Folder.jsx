...
import { CollegeFormat_for_City_District_State_UT_in_Home_Folder } from "./CollegeFormat_for_City_District_State_UT_in_Home_Folder";
import { Pre_Loader } from "./Pre_Loader";
import axios from "axios";

export const Colleges_in_City_District_State_UT_in_Home_Folder = () => {
  ...
  useEffect(() => {
    const filteredData = datas.filter(
      (item) =>
        item.city.toUpperCase() === uppercase_state ||
        item.district_name.toUpperCase() === uppercase_state ||
        item.state_or_ut_name.toUpperCase() === uppercase_state
    );
                  ...
                  {filteredDatas.length > 0 ? (
                    <ul className="w-full h-full flex justify-center items-center flex-col">
                      {filteredDatas.map((item) => {
                        const link = `/All_Colleges/List_of_Colleges/${item.name}`;
                        const text_from_All_Colleges = "All Colleges";
                        const goto_url = "/All_Colleges/List_of_Colleges";
                        return (
                          <div className="w-full h-full flex justify-center items-start flex-col bg-white sm:p-2 p-1 my-1 shadow-lg">
                            <CollegeFormat_for_City_District_State_UT_in_Home_Folder
                              data={item}
