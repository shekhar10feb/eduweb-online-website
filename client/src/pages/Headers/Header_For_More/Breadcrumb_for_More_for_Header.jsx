...
export const Breadcrumb_for_More_for_Header = ({ fieldname }) => {
    ...
    const filteredData = datas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) =>
            field.blogCategory.substring(0, 6) === "Others" &&
            field.blogCategory.substring(7) === fieldname
        ).length !== 0
    );
                    ...
                    <ul className="w-full h-full flex justify-center items-center flex-col">
                      {filteredDatas.map((item) =>
                        item.writtenBlogsByAuthor.map((topic) => {
                          if (topic.blogCategory.substring(7) === fieldname) {
