...
import { BlogFormat_for_ABOUT } from "../BlogFormat_for_ABOUT";
...
import { FormatForNoData_for_ABOUT } from "../FormatForNoData_for_ABOUT";
...
export const ABOUT_DESIGN = () => {
    ...
    const filteredData = datas.filter(
      (item) =>
        item.writtenBlogsByAuthor.filter(
          (field) => field.blogCategory === "About Design"
        ).length !== 0
    );
  ...
  if (datas.length === 0) {
    return (
      <FormatForNoData_for_ABOUT
        nav_title="Fashion Design"
        main_title="About Fashion Design"
                    ...
                    <ul className="w-full h-full flex justify-center items-center flex-col">
                      {filteredDatas.map((item) =>
                        item.writtenBlogsByAuthor.map((topic) => {
                          ...
                          let result = category.match("About Design");
                          if (result) {
                            return (
                              <div
