...
import { CollegeFormat_for_single_college } from "./CollegeFormat_for_single_college";

export const LearnMoreAboutCollege = () => {
  ...
  const { learnmoreaboutcollege } = useParams();
  ...
  useEffect(() => {
    setIsLoading(true);
    try {
      if (data.name.trim().toUpperCase() === learnmoreaboutcollege) {
        setDatas(data);
      }
    } catch (error) {
      return null;
