...
const insertCollege = async (req, res) => {
  try {
    ...
    if (collegeData) {
      return res
        .status(500)
        .json({ message: "College already exists. Please Login!" });
    }
    ...
    if (!filenameLocalPath && !tel && !email) {
      ...
      return res
        .status(200)
        .json({ message: "College is registered successfully!" });
    } else {
      if (!tel.includes("-")) {
        return res.status(500).json({
          message:
    ...
    return res
      .status(500)
      .json({ message: "College registration has failed!" });
  }
};
...
const verifyLogin = async (req, res) => {
  try {
    ...
    // const authorData = await College.findOne({ username: username });
    const collegeData = await College.findOne({ username });
...
const collegeLogout = async (req, res) => {
  try {
    req.session.destroy();
    return res.redirect("/api/loginForCollege");
  } catch (error) {
    console.log(
