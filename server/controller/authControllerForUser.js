...
const insertUser = async (req, res) => {
  try {
    ...
    if (!(username.length >= 5)) {
      return res
        .status(500)
        .json({ message: "Username must be greater than 5 letters!" });
    }

    if (!(name.length >= 5)) {
      return res
        .status(500)
        .json({ message: "Name must be greater than 5 letters!" });
    }
    ...
    return res.status(500).json({ message: "User registration has failed!" });
  }
};
...
const search_Colleges_Exams_Courses_by_user_post_method = async (req, res) => {
  ...
  try {
    ...
    let result = college.filter((u) => u.name.toUpperCase() == search_query);
    ...
    if (
      ...
      return res.status(302).redirect("/search_All_Colleges");
    } else if (
      ...
      // console.log("engg.");
      return res.status(302).redirect("/engineering_colleges");
    } else if (search_query == "MEDICAL") {
      // console.log("MEDICAL.");
      return res.status(302).redirect("/medical_colleges");
    } else if (search_query.includes("DESIGN")) {
      // console.log("DESIGN.");
      return res.status(302).redirect("/designing_colleges");
    } else if (search_query == "MANAGEMENT" || search_query == "MBA") {
      // console.log("MANAGEMENT.");
      return res.status(302).redirect("/management_colleges");
    } else if (result.length !== 0) {
      // console.log(result);
      return res.status(302).redirect(`/search_All_Colleges/${result[0].name}`);
...
const college_information = async (req, res) => {
  try {
    ...
    // console.log('collegeData: ', collegeData[0]);
...
const star_rating_given_by_user_to_college = async (req, res) => {
  try {
    ...
    // console.log("star_rating: ", trimmed_star_rating);
    // console.log("college_id: ", trimmed_college_id);

    const star_rating_by_user = parseInt(Math.ceil(trimmed_star_rating)) + "/5";
    // console.log("star_rating_by_user: ", star_rating_by_user);
    // console.log(typeof star_rating_by_user);

    const overall_number_people_who_rated =
      college[0].number_people_who_rated + 1;
    // console.log("People who rated: ", overall_number_people_who_rated);

    const overall_total_star_rating =
      college[0].total_star_rating + Number(trimmed_star_rating);
    // console.log("overall_total_star_rating: ", overall_total_star_rating);

    const percentage_of_college_rating =
      (overall_total_star_rating / (5 * overall_number_people_who_rated)) * 100;
    // console.log("percentage_of_college_rating: ", percentage_of_college_rating);

    const overall_rating1 = (percentage_of_college_rating / 100) * 5;
    // console.log("overall_rating1: ", overall_rating1);
...
const updateName = async (req, res) => {
  try {
    ...
    // console.log("Name to be changed: ", name);
...
const updateEmail = async (req, res) => {
  try {
    ...
    // console.log("Email to be changed: ", email);
...
const updateMobile = async (req, res) => {
  try {
    ...
    // console.log("Phone to be changed: ", mobile);
...
const updateUsername = async (req, res) => {
  try {
    ...
    // console.log("username to be changed: ", username);
