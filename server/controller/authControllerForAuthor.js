...
const insertAuthor = async (req, res) => {
  try {
    ...
    if (authorData) {
      return res
        .status(500)
        .json({ message: "Author already exists. Please Login!" });
    }
    ...
    return res
      .status(200)
      .json({ message: "Author is registered successfully!" });
  } catch (error) {
    console.log(
      "This is the error of authControllerForAuthorNew.js > insertUser > newAuthorData",
      error
    );
    return res.status(500).json({ message: "Author registration has failed!" });
  }
};
  ...
  try {
    ...
    if (authorData) {
      ...
      if (passwordMatch) {
        ...
        // console.log("Author Id: ", req.session.author_id);

        // const authorData1 = await Author.findOne({ _id: req.session.author_id });
        // console.log(authorData1);

        // res.render("homeForAuthor", { user: authorData1 });
...
const authorLogout = async (req, res) => {
  try {
    req.session.destroy();
    return res.redirect("/api/loginForAuthor");
  } catch (error) {
    console.log(
