...
router.get("/home", auth.isLogInForUser, userController.loadHome);

// get method for user profile route
router.get("/profile", auth.isLogInForUser, userController.loadProfile);

// get method for user settings route
router.get("/settings", auth.isLogInForUser, userController.loadSettings);
...
router.get("/blogs", auth.isLogInForUser, userController.gettingAllBlogs);
...
router.post(
  "/registerForMember",
  multer_for_member_for_registration.single("profile_image"),
...
//   "/deleteForMember/:id",
//   authForMember.isLogInForMember,
//   userControllerForMember.deleteUser
// );

// const uploadQuestionPapers_form = multer({
//   storage: uploadQuestionPapers,
...
router.post(
  ...
  uploadQuestionPapers.single("question_paper"),
...
// const upload_Ads_form = multer({
// storage: upload_Ads,

// This limits file size to 1 million bytes(1mb) fileFilter:
// limits: { fileSize: 10000000 },

// Checking the format of the files that are uploaded to the server.
// If they are in the approved format we accept otherwire we give an error to the user
// fileFilter: function (req, file, cb) {
//   if (!file.originalname.match(/\.(png|jpg|jpeg)$/)) {
//     return cb(new Error("Only png or jpg or jpeg types of file is allowed"));
//   }
//   cb(null, true);
// },
// });
...
router.post(
  ...
  // ]),
  multerForCollegeProfileImage.single("profile_image"),
...
router.post(
  ...
  multerForCollegeCoverImage.single("cover_image"),
  authForCollege.isLogInForCollege,
...
router.post(
  ...
  multerForCollegeDetailsBrochure.single("college_details_brochure"),
  authForCollege.isLogInForCollege,
...
// try {
// const url = `https://newsdata.io/api/1/news?apikey=${process.env.NEWS_API_KEY}&q=${process.env.SEARCH_TERM}%20in%20india&country=in`;
// const url = `https://newsdata.io/api/1/news?apikey=${process.env.NEWS_API_KEY_FOR_TESTING}&q=${process.env.SEARCH_TERM}%20in%20india&country=in`;
// const url = `https://newsapi.org/v2/news?apikey=${process.env.NEWS_API_KEY_FOR_TESTING}&q=${process.env.SEARCH_TERM}%20in%20india&country=in`
